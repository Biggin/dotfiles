...
if [ $# == 1 ]; then
    echo -ne "${Red}[!] --  Please supply the git username that you want to load the config for...  --${Res}"
## Script for handling multiple git identities
    if [[ ${1} == "clnelson" ]]; then
	    git config --global user.name "clnelson"
	    git config --global user.email \
		  "dumb.droid.dev@outlook.com"
    elif [[ ${1} == "Biggin" || ${1} == "biggin" ]]; then
	...
	echo -ne ${Bwhi}"[!]  --  Setting up git identity!  --\n\n"
	sleep 2
...
echo -e ${Yel}"Configuring git settings..."${Res}
sleep 2
...
git config --global color.pager "enable"
...
echo -e ${Bblu}"$(git config --list)"${Res}
