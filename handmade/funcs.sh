...
# Linux terminal if tput isn't available or if the system
# don't support 256 colors.
source "$OSH_CUSTOM/helpers/256colors"

# Script to allow showing an animated progress bar for
# drawn-out tasks in the terminal...
...
function untar () {
	local tmpdir=$HOME/.local/tmp

	tar -xf $1 &> /dev/null || _error "Invalid archive format: $1" >2
	ls -AFC --color && return 0
}
