...
function resub () {
    repo sync --force-sync --no-clone-bundle --current-branch --no-tags --submodules --optimized-fetch -j4 "$@"
}
...
#--reference=${AOSP_MIRROR} --dissociate
}
...
#--reference=${LOS_MIRROR} --dissociate
}
